...
import { actionAddBussinessemployee, actionEditBussinessemployee } from "./BussinessemployeeAction";
import { getCenterList } from "../center/CenterAction";
import Select, { components } from "react-select";
import Flatpickr from "react-flatpickr";
import "@styles/react/libs/flatpickr/flatpickr.scss";
import moment from "moment";
function AddOrEditBussinessemployeeModal(props) {
  ...
  const onSummit = async () => {
    ...
    if (isAddNew) {
      await actionAddBussinessemployee(data);
      toastSuccess("Thêm mới nhân viên kinh doanh thành công");
    } else {
      await actionEditBussinessemployee(data, item?.id);
      toastSuccess("Cập nhật nhân viên kinh doanh thành công");
    }
    onCancel(true);
  };
    ...
    <div>
      <Modal size="lg" isOpen={visible} toggle={() => onCancel(true)}>
        <ModalHeader toggle={() => onCancel(true)}>
          {!isAddNew ? "Cập nhật" : "Thêm mới"} nhân viên kinh doanh
        </ModalHeader>
        <ModalBody>
          <Form>
            <Row>
...
export default AddOrEditBussinessemployeeModal;
