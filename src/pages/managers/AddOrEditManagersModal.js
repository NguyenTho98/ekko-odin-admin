...
function AddOrEditManagersModal(props) {
  ...
  const onSummit = async () => {
    ...
    if (isAddNew) {
      await actionAddManagers(data);
      toastSuccess("Thêm mới quản lý thành công");
    } else {
      await actionEditManagers(data, item?.id);
    ...
    <div>
      <Modal size="lg" isOpen={visible} toggle={() => onCancel(true)}>
        <ModalHeader toggle={() => onCancel(true)}>
          {!isAddNew ? "Cập nhật" : "Thêm mới"} quản lý
        </ModalHeader>
        <ModalBody>
          <Form>
            <Row>
...
export default AddOrEditManagersModal;
