...
import { actionGetManagerss } from "./ManagersAction";
// ** Styles
...
import AddOrEditManagersModal from "./AddOrEditManagersModal"
...
const CustomHeader = ({
  ...
  return (
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        <Col lg="6" className="d-flex align-items-managers px-0 px-lg-1">
          <div className="d-flex align-items-managers mr-2">
        ...
        <Col
          lg="6"
          className="actions-right d-flex align-items-managers justify-content-lg-end flex-lg-nowrap flex-wrap mt-lg-0 mt-1 pr-lg-1 p-0"
        >
          <div className="d-flex align-items-managers">
            ...
            <Input
              ...
              placeholder="Tìm kiếm tên quản lý"
            />
          </div>
          {/* <Input
            className="w-auto "
...
const Managers = (props) => {
  const { profile, actionGetManagerss, managerss = {}, isFetching } = props;
  ...
  useEffect(() => {
    if (!isEmpty(profile)) {
      handleFetchManagers(params);
    }
  }, [profile]);

  const handleFetchManagers = (params = {}) => {
    actionGetManagerss({ ...params });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      cell: row => (
        <div className='column-action d-flex align-items-managers'>
  ...
  const handlePerPage = (e) => {
    ...
    //   })
    // );
    actionGetManagerss({ ...params, limit:e.target.value })
  ...
  const handlePagination = (page) => {
    actionGetManagerss({ ...params, limit: rowsPerPage, offset: (page.selected) * rowsPerPage })
  ...
  const CustomPagination = () => {
    const count = Number((managerss.count / rowsPerPage).toFixed(0));
    return (
      <ReactPaginate
        pageCount={count || 1}
        nextLabel=""
  ...
  return (
    <Fragment>
      <Breadcrumbs breadCrumbTitle="Quản lý" />
      ...
      {visibleModalDelete && (
        <DeleteManagersModal
          ...
          handleFetchManagers={() => handleFetchManagers(params)}
...
export default connect(
    (state) => ({
      ...
      managerss: state.managers?.managerss,
