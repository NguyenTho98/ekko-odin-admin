...
import { actionAddComment, actionDeleteComment, actionEditComment } from "./CommentAction";

function DeleteCommentModal(props) {
  const { visible, onCancel, item = {}, handleFetchComment } = props;
  const onSummit = async () => {
    await actionDeleteComment(item.id);
    toastSuccess("Xoá bình luận thành công")
    handleFetchComment();
    onCancel(true);
  }
  return (
    <div>
      <Modal isOpen={visible} toggle={() => onCancel(true) }>
        <ModalHeader toggle={() => onCancel(true) }>Xóa bình luận {item.name}</ModalHeader>
        <ModalBody>
          <Form>
            <Row>
              <Col sm="12">
