...
import { actionAddTeachers, actionDeleteTeachers, actionEditTeachers } from "./TeachersAction";

function DeleteTeachersModal(props) {
  const { visible, onCancel, item = {}, handleFetchTeachers } = props;
  const onSummit = async () => {
    await actionDeleteTeachers(item.id);
    toastSuccess("Xoá giáo viên thành công")
    handleFetchTeachers();
    onCancel(true);
  }
  return (
    <div>
      <Modal isOpen={visible} toggle={() => onCancel(true) }>
        <ModalHeader toggle={() => onCancel(true) }>Xóa giáo viên {item.name}</ModalHeader>
        <ModalBody>
          <Form>
            <Row>
              <Col sm="12">
