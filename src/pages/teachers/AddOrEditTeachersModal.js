...
import { actionAddTeachers, actionEditTeachers } from "./TeachersAction";
import { getCenterList } from "../center/CenterAction";
import Select, { components } from "react-select";
import Flatpickr from "react-flatpickr";
import "@styles/react/libs/flatpickr/flatpickr.scss";
import moment from "moment";
function AddOrEditTeachersModal(props) {
  ...
  const onSummit = async () => {
    ...
    if (isAddNew) {
      await actionAddTeachers(data);
      toastSuccess("Thêm mới giáo viên thành công");
    } else {
      await actionEditTeachers(data, item?.id);
      toastSuccess("Cập nhật giáo viên thành công");
    }
    onCancel(true);
  };
    ...
    <div>
      <Modal size="lg" isOpen={visible} toggle={() => onCancel(true)}>
        <ModalHeader toggle={() => onCancel(true)}>
          {!isAddNew ? "Cập nhật" : "Thêm mới"} giáo viên
        </ModalHeader>
        <ModalBody>
          <Form>
            <Row>
...
export default AddOrEditTeachersModal;
