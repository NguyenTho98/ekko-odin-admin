...
import { actionAddStudentcare, actionEditStudentcare } from "./StudentcareAction";
...
function AddOrEditStudentcareModal(props) {
  ...
  const onSummit = async () => {
    ...
    if (isAddNew) {
      await actionAddStudentcare(data);
      toastSuccess("Thêm mới chăm sóc học viên thành công");
    } else {
      await actionEditStudentcare(data, item?.id);
      toastSuccess("Cập nhật chăm sóc học viên thành công");
    }
    onCancel(true);
  };
    ...
    <div>
      <Modal size="lg" isOpen={visible} toggle={() => onCancel(true)}>
        <ModalHeader toggle={() => onCancel(true)}>
          {!isAddNew ? "Cập nhật" : "Thêm mới"} chăm sóc học viên
        </ModalHeader>
        <ModalBody>
          <Form>
            <Row>
...
export default AddOrEditStudentcareModal;
